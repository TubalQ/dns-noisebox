...
# DNS resolvers to query (e.g., Cloudflare, Technitium, Quad9)
RESOLVERS=("9.9.9.9" "1.1.1.1" "8.8.8.8" "94.140.14.14" "208.67.222.222")

# Number of queries per run
NUM_QUERIES=40

# Wordlist for generating random domains
WORDLIST="/opt/noicebox/words.txt"

# Log file
LOGFILE="/var/log/dns_noise.log"
MAXSIZE=50000  # Max size in bytes (~50 KB)
...
for i in $(seq 1 $NUM_QUERIES); do
  ...
  if (( RANDOM % 10 < 7 )); then
    ...
    DOMAIN=$(shuf -n 1 -e "${REAL_DOMAINS[@]}")
    TYPE="REAL"
  fi

  # Select a random resolver
  RESOLVER=$(shuf -n 1 -e "${RESOLVERS[@]}")

  # Query using dig with timeout and retry limit
  dig @"$RESOLVER" "$DOMAIN" +timeout=2 +tries=1 +short > /dev/null

  # Write to log
  echo "$TIMESTAMP [$TYPE] ($RESOLVER) $DOMAIN" >> "$LOGFILE"

  # Random delay between queries (0.5–2.5 seconds)
